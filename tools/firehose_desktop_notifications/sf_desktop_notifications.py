...
NOTIFICATION_ICONS['heart_green'] = GdkPixbuf.Pixbuf.new_from_file(_heart_green)
...
class Filter:
    def __init__(self, topic, keyvals, msg_format=None, icon=None):
        self.topic = topic
        self.criterias = {}
    ...
    def check(self, topic, msg):
        ...
        if not all(k in flat_msg for k in self.criterias):
            LOGGER.debug("missing criterias")
            return False
        if all(self.criterias[k].search(flat_msg[k]) for k in self.criterias):
            LOGGER.debug("failing regexps")
            return True

    def format(self, topic, msg):
        flat_msg = flatten_dict(msg)
        flat_msg.update({'topic': topic})
        return self.msg_format % flat_msg
...
def on_message(filters):
    def _on_message(client, userdata, msg):
        ...
        for filter in filters:
            if filter.check(msg.topic, jmsg):
                try:
                    message = filter.format(msg.topic, flatten_dict(jmsg))
                except KeyError as e:
                    LOGGER.debug('%s missing key %s' % (jmsg, e))
                Notify.init('Software Factory message')
                n = Notify.Notification.new(
                    'Software Factory',
                    message)
                n.set_icon_from_pixbuf(filter.icon)
                n.set_image_from_pixbuf(filter.icon)
                n.show()
                return
    return _on_message
...
def main():
    ...
    if 'filters' in conf:
        for filter in conf['filters']:
            ...
            filters.append(Filter(topic, keyvals, msg_format, icon))
