...
class StoryboardManager(object):
    def __init__(self, url, api_key, project_group, board=None,
        ...
        self.client = SFStoryboard(urljoin(url, "storyboard_api"),
                                   self.cookie)
        try:
            self.project_group = self.client.project_groups.find(name=project_group)
        except exceptions.NotFound:
            raise Exception('projects group not found')
        self.stories = self.client.stories.get_all(project_group_id=self.project_group.id)
        self.board_id = None
        if board:
            try:
        ...
        if groom_worklist:
            try:
                self.groom_worklist = self.client.worklists.find(title=groom_worklist)
            except exceptions.NotFound:
                raise Exception('unrefined stories worklist not found')
        if bug_worklist:
            try:
                self.bug_worklist = self.client.worklists.find(title=bug_worklist)
            except exceptions.NotFound:
                raise Exception('unrefined stories worklist not found')
...
#        return task.project_id in self.projects_ids
    ...
    def find_current_lane(self, item, type):
        lanes = self.board_lanes.values() + [self.groom_worklist, self.bug_worklist, ]
        for lane in lanes:
            for i in lane.items:
                if i['item_type'] == type and i['item_id'] == item.id and not i['archived']:
                    return lane.id, i['id']
        return None, None
    ...
    def move_to_lane(self, item, type, lane_id=None):
        ...
        if current_lane:
            print "#%s found in worklist '%s'," % (item.id, self.client.worklists.get(id=current_lane).title),
    ...
    def is_to_groom(self, story, regroom_after=None):
        # find stories in project group that are untagged, unassigned, in status "Todo"
        ...
        if all(t.assignee_id == None for t in tasks):
            if all(t.status.lower() == 'todo' for t in tasks):
                ...
                if regroom_after:
                    ...
                    last_update = datetime.datetime.strptime(story.updated_at,
                                                             '%Y-%m-%dT%H:%M:%S+00:00')
    ...
    def is_to_triage(self, story):
        # find stories in project group that are tagged "bug" but not tagged "confirmed"
        if 'bug' in story.tags and 'confirmed' not in story.tags and story.status.lower() == 'active':
            return True
        return False

    def is_open_task(self, task):
        # find unassigned tasks in todo status from stories in project group that are tagged "groomed" (assumed), not tagged "blocked"
    ...
    def is_open_confirmed_bug(self, task):
        # find unassigned tasks in todo status from stories in project group that are tagged "bug" & "confirmed"
        if task.status.lower() == "todo" and task.assignee_id == None:
            if set(["bug", "confirmed"]) in set(self.client.stories.get(id=task.story_id).tags):
                return True
        return False

    def is_current_sprint(self, task):
        # find assigned tasks in todo status from stories in project group that are tagged "groomed" or ("bug" & "confirmed")
        if task.status.lower() == "todo" and task.assignee_id != None:
            return True
        return False
    ...
    def is_done_during_sprint(self, task):
        # tasks in project groups that were set as "Done" before current due date, after previous one 
    ...
    def set_due_date_for_sprint(self, sprint_name):
        ...
        sprint_end = datetime.datetime.strptime(sprint_name,'%Y-%W-%w').strftime('%Y-%m-%dT23:59:00+00:00')
        self.client.due_dates.create(name=sprint_name, date=sprint_end, board_id=self.board_id)
        ...
        for story in self.stories:
            ...
            if self.is_to_triage(story):
                ...
                if story.status.lower() == 'active':
                    self.move_to_lane(story, 'story', self.board_lanes['Groomed Stories'].id)
                ...
                for task in story.tasks.get_all():
                    ...
                    if self.is_blocked_task(task):
                        ...
                        print "Task #%s (story #%s) is blocked" % (task.id, story.id)
                        self.move_to_lane(task, 'task', self.board_lanes['Blocked'].id)
                    elif self.is_open_confirmed_bug(task):
                        # move to lane 'Open Confirmed Bugs'
                        print "Task #%s (story #%s) is an open, confirmed bug" % (task.id, story.id)
                        self.move_to_lane(task, 'task', self.board_lanes['Open Confirmed Bugs'].id)
...
def main():
    ...
    update_boards = boards_subcmd.add_parser('update',
                                             help='Update boards lanes & worklists')
    update_boards.add_argument('--board', metavar='BOARD', default='sf-sprint',
                               help='The board to use')
    update_boards.add_argument('--bug-worklist', metavar='BUG-WORKLIST', default='sf-bug-triage',
                               help='The worklist to use for bug triage')
    update_boards.add_argument('--unrefined-worklist', metavar='UNREFINED-WORKLIST',
                               default='sf-unrefined-stories',
                               help='The bug worklist to use')
    clean_boards = boards_subcmd.add_parser('clean',
                                            help='Remove tasks and stories that do not belong in boards lanes & worklists')
    clean_boards.add_argument('--board', metavar='BOARD', default='sf-sprint',
                              help='The board to use')
    clean_boards.add_argument('--bug-worklist', metavar='BUG-WORKLIST', default='sf-bug-triage',
                              help='The worklist to use for bug triage')
    clean_boards.add_argument('--unrefined-worklist', metavar='UNREFINED-WORKLIST',
                              default='sf-unrefined-stories',
                              help='The bug worklist to use')
    # sprint commands
    ...
    start_sprint.add_argument('--board', '-b', metavar='BOARD', default='sf-backlog-ng',
                              help='The board to use')
    ...
    summary_sprint = sprint_subcmd.add_parser('summary',
                                              help='Generate a summary of a sprint')
