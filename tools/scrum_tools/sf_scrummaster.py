...
class StoryboardManager(object):
    ...
    def remove_from_lane(self, item, type, lane_id):
        print "removing %s %s from %s" % (type, item.id, lane_id)
    ...
    def is_open_confirmed_bug(self, task):
        ...
        if task.status.lower() == "todo" and task.assignee_id is None:
            tags_set = set(self.client.stories.get(id=task.story_id).tags)
            if set(["bug", "confirmed"]) in tags_set:
                return True
        return False
    ...
    def clean_groom_worklist(self):
        ...
        print "Cleaning groom worklist..."
        stories = self.client.worklists.get(id=self.groom_worklist.id).items
        for story in stories:
            if story['archived']:
                print "story %s archived, nothing to do" % story['item_id']
            s = self.client.stories.get(story['item_id'])
            if not self.is_to_groom(s):
                self.remove_from_lane(s, 'story', self.groom_worklist.id)

    def clean_bug_worklist(self):
        print "Cleaning bug worklist..."
        ...
        for story in self.client.worklists.get(id=self.bug_worklist.id).items:
            ...
            if not self.is_to_triage(s):
                self.remove_from_lane(s, 'story', self.bug_worklist.id)
    ...
    def clean_sprint_board(self):
        ...
        for story in self.client.worklists.get(lane_id).items:
            ...
            if s.status.lower() != 'active':
                self.remove_from_lane(s, 'story', lane_id)
        # Open Tasks
        ...
        for task in self.client.worklists.get(lane_id).items:
            ...
            if t.status.lower() != 'todo' or t.assignee_id is not None:
                self.remove_from_lane(t, 'task', lane_id)
        # Open Confirmed Bugs
...
def main():
    ...
    summary_sprint = sprint_subcmd.add_parser('summary',
                                              help='Generate a summary '
                                                   'of current sprint')
    summary_sprint.add_argument('--board', '-b', metavar='BOARD',
                                default='sf-backlog-ng',
                                help='The board to use')
    ...
    if args.command == 'boards':
        ...
        if args.subcommand == 'start':
            ...
            for lane in ['Current Sprint',
                         ...
                         'Ready for Review',
                         'Done']:
                for task in manager.boards[lane].items:
