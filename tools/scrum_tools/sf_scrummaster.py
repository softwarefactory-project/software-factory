...
class StoryboardManager(object):
    ...
    def update_boards(self):
        ...
        for story in self.stories:
            ...
            if not self.is_to_triage(story) and not self.is_to_groom(story):
...
def main():
    ...
    if args.command == 'boards':
        ...
        if args.subcommand == 'start':
            ...
            for task in manager.boards['Done'].items:
                if 'archived' in task and not task['archived']:
                    if task['updated_at'] < start_date:
                        t = manager.client.tasks.get(id=task['item_id'])
                        manager.remove_from_lane(t, 'task', 'Done')
